...
(async () => {
  ...
  const readHud = async () => page.evaluate(() => {
    ...
    return {
      guidance: g ? { bearing: g[1], dist: parseFloat(g[2]), units: parseInt(g[3]) } : null,
  ...
  while (Date.now() - start < 200000) {
    ...
    if (ticks % 10 === 0) console.log(`t=${((Date.now() - start) / 1000).toFixed(0)}s`, JSON.stringify(hud), 'phase:', phase);
    ...
    if (phase === 'toSalvage') {
      if (!hud.guidance) { console.log('no guidance visible'); }
      ...
      if (lastDist !== null && d !== null) {
        if (d < lastDist - 0.05) { noProgress = 0; }
