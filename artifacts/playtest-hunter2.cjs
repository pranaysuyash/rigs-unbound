...
(async () => {
  ...
  while (Date.now() - t0 < 220000) {
    ...
    if (h0.salvage !== null && h0.salvage > lastSalvage) {
      ...
      console.log(`*** SALVAGE = ${lastSalvage} at t=${((Date.now() - t0) / 1000).toFixed(0)}s`);
      await shot(`50-salvage-${lastSalvage}`);
    }
    if (h0.dist === null) { await drive(['w'], 800); continue; }
    if (h0.dist < 3.5) {
    ...
    if (h3 && h3.dist !== null) console.log(`steer: ${h0.dist.toFixed(1)} -> ${h3.dist.toFixed(1)} bearing=${h3.bearing} units=${h3.units} spd=${h3.speed}`);
  }
