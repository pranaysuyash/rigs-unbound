...
(async () => {
  const browser = await chromium.launch({
    headless: true,
    executablePath:
      "/Users/pranay/Library/Caches/ms-playwright/chromium-1223/chrome-mac-arm64/Google Chrome for Testing.app/Contents/MacOS/Google Chrome for Testing",
  });
  ...
  for (const step of plan.steps) {
    try {
      if (step.wait) {
        ...
        await page.keyboard.press(step.tap);
        await page.waitForTimeout(step.after || 150);
      } else if (step.click) {
        ...
        await page.mouse.move(step.drag[0], step.drag[1]);
        await page.mouse.down();
        await page.mouse.move(step.drag[2], step.drag[3], { steps: 10 });
        await page.mouse.up();
      } else if (step.text) {
        ...
        console.log("TEXT>>>", String(t).slice(0, 4000), "<<<TEXT");
      } else if (step.eval) {
        const r = await page.evaluate(step.eval);
        console.log("EVAL>>>", JSON.stringify(r)?.slice(0, 2000), "<<<EVAL");
      } else if (step.reload) {
        await page.reload({ waitUntil: "networkidle" }).catch(() => {});
        await page.waitForTimeout(3000);
      }
    } catch (e) {
      console.log("step error:", step, e.message);
    }
  }
