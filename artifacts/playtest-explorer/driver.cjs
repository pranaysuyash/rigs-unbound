...
(async () => {
  ...
  page.on("console", (m) => {
    if (m.type() === "error")
      out({ err: "CONSOLE: " + m.text().slice(0, 300) });
  });
  ...
  async function exec(cmd) {
    try {
      switch (cmd.op) {
        case "shot":
          await page.screenshot({ path: `${DIR}/${cmd.name}.png` });
          out({ ok: "shot " + cmd.name });
          break;
        case "wait":
        ...
        case "keydown":
          ...
          keysDown.add(cmd.key);
          out({ ok: "down " + cmd.key });
          break;
        case "keyup":
          await page.keyboard.up(cmd.key);
          keysDown.delete(cmd.key);
          out({ ok: "up " + cmd.key });
          break;
        case "hold":
          await page.keyboard.down(cmd.key);
        ...
        case "press":
          ...
          out({ ok: "pressed " + cmd.key });
          break;
        case "click":
          await page.click(`text=${cmd.text}`, { timeout: 3000 });
          out({ ok: "clicked " + cmd.text });
          break;
        case "clickxy":
          ...
          break;
        case "move":
          await page.mouse.move(cmd.x, cmd.y, { steps: 10 });
          out({ ok: "move" });
          break;
        case "text": {
          const t = await page.evaluate(() => document.body.innerText);
          out({ text: t.slice(0, cmd.n || 2500) });
          break;
        }
        case "rgt": {
          const t = await page.evaluate(() => window.render_game_to_text());
          out({ rgt: String(t).slice(0, cmd.n || 3000) });
          break;
        }
        case "eval": {
          const r = await page.evaluate(cmd.js);
          out({ eval: JSON.stringify(r)?.slice(0, 2000) });
          break;
        }
        case "releaseAll":
          ...
          break;
        default:
          out({ err: "unknown op " + cmd.op });
      }
    } catch (e) {
      out({ err: cmd.op + " failed: " + e.message.slice(0, 300) });
    }
  }
