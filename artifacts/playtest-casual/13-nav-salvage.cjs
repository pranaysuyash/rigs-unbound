...
(async () => {
  ...
  const readHud = async () => {
    ...
    const m = txt.match(
      /([NSEW]+)\s*·\s*salvage\s*(\d+)\s*m\s*·\s*(\d+)\s*unit/i,
    );
    const rig = txt.match(/(Torque|Spark|Drift)\n/);
    return m
      ? { dir: m[1], dist: +m[2], units: +m[3], rig: rig && rig[1] }
      : { raw: txt.slice(0, 200), rig: rig && rig[1] };
  };
  ...
  for (let i = 0; i < 45; i++) {
    ...
    if (!before.dist) {
      console.log(i, "no hud", JSON.stringify(before));
      break;
    }
    if (before.dist <= 5) {
      console.log(i, "CLOSE ENOUGH", JSON.stringify(before));
      break;
    }
