...
(async () => {
  ...
  const modalVisible = await page.getByText('ENTER THE FIELD').isVisible().catch(() => false);
  console.log('intro modal visible after reload:', modalVisible);
  if (modalVisible) { await page.getByText('ENTER THE FIELD').click(); await page.waitForTimeout(1000); }
