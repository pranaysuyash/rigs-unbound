...
import { adjustTirePressure, calculateTirePressureState } from "./tire-pressure";
...
describe("tire pressure & contact patch engine", () => {
  it("expands contact patch area and lowers ground pressure when airing down", () => {
    ...
    expect(airedDown.contactPatchAreaM2).toBeGreaterThan(highway.contactPatchAreaM2);
    expect(airedDown.groundPressureKpa).toBeLessThan(highway.groundPressureKpa);
    expect(airedDown.mudFloatationFactor).toBeGreaterThan(highway.mudFloatationFactor);
  });
