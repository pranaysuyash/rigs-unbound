...
import { createInitialState, installModule } from "./state";
...
describe("first progression rung", () => {
  ...
  it("guides an incompatible active rig to a compatible rig without lying", () => {
    ...
    expect(result.objective).toContain("Switch rig");
  });
  ...
  it("makes the recommended first fit mechanically and visibly meaningful", () => {
    ...
    installModule(state, world, FIRST_RUNG_RECOMMENDED_MODULE);
    ...
    expect(resolveFirstRung(state, world.collectedNodes).complete).toBe(true);
    expect(state.lastDiagnostic).toContain("Bites into mud");
  });
