...
describe("campaign contracts engine", () => {
  ...
  it("locks follow-up campaign contracts until the relay route is completed", () => {
    ...
    expect(missions.some((m) => m.id === "contract-sunken-relay")).toBe(true);
  ...
  it("unlocks chained campaign contracts after the root contract completes", () => {
    ...
    const completed = completeMission(state, relay.id, state.elapsedMs + 1000);
    expect(completed.ok).toBe(true);

    const after = deriveMissions(state, state.progression, "clear", new Set());
    expect(after.some((m) => m.id === "contract-ridge-ascent")).toBe(true);
  });

  it("keeps authored campaign data in sync with proposition expectations", () => {
    expect(
      CAMPAIGN_CONTRACTS.some((c) => c.id === "contract-sunken-relay"),
    ).toBe(true);
    expect(
      CAMPAIGN_CONTRACTS.some((c) => c.id === "contract-ridge-ascent"),
    ).toBe(true);
  });
});
