...
| ADR-0042 | Implemented and verified for the current runtime | Persistent infrastructure remains autonomous and spatial; Sunken Flats waterworks identity (ADR-0052) handles regional water mechanics. |
| ADR-0052 | Implemented and verified for the current runtime | Sunken Flats Waterworks supersede the Floodgate singleton as the canonical regional infrastructure identity, with legacy-save migration. Verified in state.test.ts and browser acceptance. |
| ADR-0043 | Accepted by direct operator direction; implemented and verified | Communities publish persistent material needs and favor through the canonical mission pipeline. Shipped and verified. |
| ADR-0044 | Implemented and verified for the current runtime | Runtime bridge fallback meshes and loaded GLTF roots are explicitly disposed at teardown. |
| ADR-0045 | Implemented and verified for the current runtime | Grove Run voluntary repeatable open-road activity with per-rig records. Verified in activities.test.ts. |
| ADR-0046 | Implemented and verified for the current runtime | Quarry Runout weather, landslide, and collision incident management without quest gates. Verified in state.ts. |
| ADR-0047 | Implemented and verified for the current runtime | Canonical asset definitions under `assets/specs/` own reusable asset semantics. Verified by asset preflight audits. |
| ADR-0048 | Implemented and verified for the current runtime | Vision-led disablement recovery constellation (home recall, rig switching, emergency recovery). Verified in state.test.ts. |
| ADR-0049 | Implemented and verified for the current runtime | Settlement contribution history records voluntary, capability-specific partial help without a mission acceptance funnel. Verified in settlement-life.test.ts. |
