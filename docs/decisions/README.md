...
| ADR-0042 | Proposed - operator sign-off required; identity surface superseded by ADR-0052 | Persistent infrastructure remains autonomous and spatial; the former Floodgate singleton identity is retained only for legacy recovery. |
| ADR-0052 | Proposed - operator sign-off required; implementation in progress | Sunken Flats Waterworks supersede the Floodgate singleton as the canonical regional infrastructure identity, with legacy-save migration. |
| ADR-0043 | Accepted by direct operator continuation; first runtime stage implemented, verification pending | Communities publish persistent material needs and favor through the canonical mission pipeline, so machine work changes places instead of advancing isolated instrument chains. |
| ADR-0044 | Implemented and verified for the current runtime | Runtime bridge fallback meshes and loaded GLTF roots are now explicitly disposed at the point they're orphaned/at teardown; corrects a code comment that incorrectly claimed `WebGLRenderer.dispose()` frees them (verified false against three's own source). |
| ADR-0045 | Proposed - operator sign-off required; source integration in progress | Grove Run introduces a voluntary, repeatable open-road activity with per-rig records, shared terrain/simulation, and no route gate, mission authority, or repeatable currency reward. Runtime evidence remains open. |
| ADR-0046 | Proposed - operator sign-off required; source integration in progress | Quarry Runout turns weather, landslide stability, debris displacement, collision, renderer props, and world-memory recovery into one optional persistent incident without a quest gate or parallel world truth. Runtime evidence remains open. |
| ADR-0047 | Proposed — operator sign-off required | Canonical asset definitions under `assets/specs/` own reusable asset semantics; img2threejs, factories, GLBs, and runtime adapters are derived compiler stages. |
| ADR-0048 | Proposed — operator sign-off required | Disablement opens a vision-led recovery constellation: home recall/teleport, character or vehicle switching, NPC/async/live help, earned rescue capacity, repair/mechanic calls, advanced self-repair, and physical stranded-rig consequence. |
...
| ADR-0053 | Accepted | Top-Down Game Mode Architecture & Control Paradigms: Defines multi-engine renderer adapters, context-adaptive steering (Vehicle vs Arcade vs Twin-Stick), 75° perspective diorama camera tuning, and 4 top-down activity archetypes. |
